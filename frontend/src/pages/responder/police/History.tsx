 ...
 * Summary KPIs + one card per completed incident with severity badge.
 */
    ...
    return (
      <div className="mx-auto max-w-lg px-4 py-8">
        <EmptyState
          ...
          description="Completed incidents will appear here"
        />
      </div>
    );
  }

  const securityCount = completedCases.filter((c) => c.type === "security").length;
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card flush className="flex flex-col items-center gap-1 p-3 text-center">
            ...
            <p className="text-xs font-semibold text-ink-muted">Incidents</p>
          </Card>
          <Card flush className="flex flex-col items-center gap-1 p-3 text-center">
            <span
              aria-hidden="true"
              className="flex h-8 w-8 items-center justify-center rounded-md bg-warning-soft text-on-warning-soft"
            >
              <Siren className="h-4 w-4" />
            </span>
            <p className="text-2xl font-bold text-ink">{securityCount}</p>
            <p className="text-xs font-semibold text-ink-muted">Security</p>
          </Card>
          <Card flush className="flex flex-col items-center gap-1 p-3 text-center">
            <span
              aria-hidden="true"
